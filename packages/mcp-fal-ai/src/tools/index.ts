...
export function registerTools(server: McpServer, token: string): void {
    ...
    server.tool(
        'download-image',
        'Download an image from a URL and return its base64 content along with the MIME type. Use this tool to fetch images and convert them to base64 format for further processing.',
        {
            url: z.string().url('Must be a valid URL'),
        },
        async ({ url }) => {
            try {
                const response = await client.downloadImage(url);
                return {
                    content: [
                        {
                            type: 'text',
    ...
    //             const status = (await client.getStatus(requestId, modelId)) as
    ...
    //                 s === 'canceled'
    //             ) {
    //                 const result = await client.getResult(requestId, modelId);
    ...
    //             content: [
    //                 { type: 'text', text: logs.join('\n') },
    //                 { type: 'text', text: toText({ error: 'timeout', requestId }) },
    //             ],
    //         };
    //     },
    // );
